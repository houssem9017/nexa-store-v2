...
import { ArrowUpRight, Cable, Gamepad2, Grid2X2, Headphones, Heart, House, Search, ShoppingBag, Smartphone, Watch } from "lucide-react";
  ...
  return (
    <main>
      ...
      <header className="siteHeader">
        ...
        <div className="headerActions"><button className="iconButton" aria-label="Rechercher" onClick={() => setSearchOpen(true)}><Search size={22} strokeWidth={1.8}/></button><a className="iconButton desktopOnly" href="/favoris" aria-label="Voir les favoris"><Heart size={20} strokeWidth={1.8} /></a><a className="iconButton cartButton" href="/panier" aria-label="Ouvrir le panier"><span className="bagIcon" />{cart.length > 0 && <b>{cart.length}</b>}</a></div>
      </header>
      ...
      <footer className="siteFooter"><div className="footerMain"><div className="footerIntro"><a className="wordmark" href="#top">nexa<span>•</span></a><p>Des objets bien choisis pour votre quotidien.</p></div><div className="footerLinks"><div><strong>Boutique</strong><a href="#produits">Nouveautés</a><a href="#produits">Best-sellers</a><a href="#produits">Offres du moment</a></div><div><strong>Aide</strong><a href="#top">Livraison</a><a href="#top">Retours</a><a href="#top">FAQ</a></div><div><strong>Suivre Nexa</strong><a href="#top">Instagram</a><a href="#top">Facebook</a><a href="#top">TikTok</a></div></div></div><div className="footerBottom"><small>© 2026 Nexa Store · Tunisie</small><a href="https://visiotec.tn" target="_blank" rel="noreferrer">Réalisé par Visiotec</a></div></footer>

      <nav className="mobileNav" aria-label="Navigation mobile"><button className={activeMobileNav === "home" ? "active" : ""} onClick={() => { setActiveMobileNav("home"); window.scrollTo({ top: 0, behavior: "smooth" }); }}><House className="mobileNavIcon" strokeWidth={1.8}/><span>Accueil</span></button><button className={activeMobileNav === "shop" ? "active" : ""} onClick={() => { setActiveMobileNav("shop"); browse(); }}><Grid2X2 className="mobileNavIcon" strokeWidth={1.8}/><span>Boutique</span></button><button className={activeMobileNav === "search" ? "active" : ""} onClick={() => { setActiveMobileNav("search"); setSearchOpen(true); }}><Search className="mobileNavIcon" strokeWidth={1.8}/><span>Recherche</span></button><a href="/favoris" aria-label="Voir les favoris" onClick={() => setActiveMobileNav("favorites")}><Heart className="mobileNavIcon" strokeWidth={1.8}/><span>Favoris</span></a><a href="/panier" aria-label="Voir le panier" onClick={() => setActiveMobileNav("cart")}><ShoppingBag className="mobileNavIcon" strokeWidth={1.8}/>{cart.length > 0 && <b>{cart.length}</b>}<span>Panier</span></a></nav>

      {searchOpen && <div className="overlay" role="dialog" aria-modal="true" aria-label="Rechercher"><div className="searchPanel"><div className="searchTop"><button className="iconButton" onClick={() => setSearchOpen(false)} aria-label="Fermer la recherche">←</button><div><span>⌕</span><input ref={searchInput} value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Rechercher un produit..." aria-label="Rechercher un produit" />{query && <button onClick={() => setQuery("")} aria-label="Effacer">×</button>}</div></div>{query ? <div className="searchResults"><p>Produits trouvés</p>{searchResults.length ? searchResults.map((product) => <a key={product.id} href={`/produit/${product.id}`} onClick={() => setSearchOpen(false)}><img src={product.image} alt="" /><span><strong>{product.name}</strong><small>{product.brand}</small></span><b>{formatPrice(product.price)}</b></a>) : <div className="emptySearch">Aucun résultat. Essayez « audio » ou «gaming».</div>}</div> : <div className="searchSuggestions"><p>Recherches populaires</p><div>{["Halo", "Pulse", "Orbit", "Rise"].map((item) => <button key={item} onClick={() => setQuery(item)}>{item} <ArrowUpRight className="suggestedArrow" strokeWidth={1.8}/></button>)}</div><p>Catégories tendances</p><div className="suggestCategories"><button onClick={() => { browse("Audio"); setSearchOpen(false); }}>Audio</button><button onClick={() => { browse("Gaming"); setSearchOpen(false); }}>Gaming</button><button onClick={() => { browse("Maison"); setSearchOpen(false); }}>Maison</button></div></div>}</div></div>}
